...
ImageCanvas::ImageCanvas(MainWindow *ui) : QLabel(), _ui(ui) {
...
void ImageCanvas::loadImage(const QString &filename) {
  ...
  _upPath =
      file.dir().absolutePath().mid(0, mask_file_length - basename_length);
  ...
  _mask_file =
      _upPath + _savePathInformation.mask + "/" + file.baseName() + "_mask.png";
  _raw_image_file =
      _upPath + _savePathInformation.raw_image + "/" + file.baseName() + ".png";

  _watershed_file = _upPath + _savePathInformation.watershed_mask + "/" +
                    file.baseName() + "_watershed_mask.png";
...
void ImageCanvas::saveMask() {
  ...
  if (!_watershed.id.isNull()) {
    ...
    QFileInfo file(_img_file);
    QString color_file = _upPath + _savePathInformation.color_mask + "/" +
                         file.baseName() + "_color_mask.png";
...
void ImageCanvas::paintEvent(QPaintEvent *event) {
  ...
  if (_mouse_pos.x() > 10 && _mouse_pos.y() > 10 &&
    ...
    painter.drawEllipse(_mouse_pos.x() / _scale - _pen_size / 2,
                        _mouse_pos.y() / _scale - _pen_size / 2, _pen_size,
                        _pen_size);
    painter.end();
  }
}
...
void ImageCanvas::mouseReleaseEvent(QMouseEvent *e) {
  ...
  if (e->button() == Qt::RightButton) { // selection of label
    ...
    const LabelInfo *label = _ui->id_labels[maskColor.red()] != NULL
                                 ? _ui->id_labels[maskColor.red()]
                                 : _ui->id_labels[watershedColor.red()];
    if (label != NULL) {
      if (!_watershed.id.isNull() &&
...
void ImageCanvas::wheelEvent(QWheelEvent *event) {
  ...
  if (Qt::ShiftModifier == event->modifiers()) {
    ...
    _scroll_parent->verticalScrollBar()->setEnabled(false);
    double value =
        _ui->spinbox_scale->value() + delta * _ui->spinbox_scale->singleStep();
